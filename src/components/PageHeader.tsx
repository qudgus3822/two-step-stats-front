...
interface PageHeaderProps {
  title: ReactNode;
  sub?: ReactNode;
  // [신설: 2026-09-03 09:00, 김병현 작성] 페이지를 상징하는 아이콘(계획서 §Phase 2-3
  // "카드 제목 옆... 아이콘을 일관되게"). 화면마다 다른 크기로 흩어지지 않도록 여기서
  // size-6 로 고정한다 — 호출부는 "어떤 아이콘인지"만 고른다.
  icon?: LucideIcon;
}

export function PageHeader({ title, sub, icon: Icon }: PageHeaderProps) {
  return (
    <div className="mb-5">
      <div className="flex items-center gap-2.5">
        {Icon && <Icon aria-hidden="true" className="size-6 shrink-0 text-primary" />}
